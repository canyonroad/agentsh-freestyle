...
> **Status update (2026-05-29):** This document captured the original implementation plan against agentsh **v0.16.9** and a Freestyle kernel without Landlock. The integration now pins **agentsh v0.20.3** on Freestyle kernels that ship **Landlock**. Code snippets below are preserved as a historical record of the initial build — for the current state see `README.md` and the "v0.18.x + Landlock Update" / "v0.20.2 Update" / "v0.20.3 Update" sections in `docs/superpowers/specs/2026-04-06-agentsh-freestyle-demo-design.md`.
